...
def post_article(session, title, excerpt, body, magic_token, publish_date, debug=False):
    # Markdown → HTML変換（本文はHTMLへ変換）
...
def main():
    ...
    parser.add_argument("--debug", action="store_true", help="デバッグ情報（送信ペイロード・レスポンス本文）を出力")
    ...
    # フィルタ：--day, --start, --end の指定に応じて対象を抽出
    filtered_files = []
    ...
    for filepath in filtered_files:
        try:
            ...
            if day_num is None:
                ...
                continue
            # 予約公開日時は、Day1なら base_date、Day N は base_date + (N - 1) 日
            ...
            response = post_article(session, title, excerpt, body, magic_token, publish_date, debug=args.debug)
